...
impl Vulkan {
	...
	pub fn destroy<T>(&self, resource: T) where T: VkDestroy {
	...
	unsafe fn unsafe_new(window: &Window) -> Self {
		...
		let physical_device = {
			...
			PhysicalDevice {
				...
				memory_properties,
				queue_family_idx: queue_family_index,
			}
		};
		...
		let queue = device.get_device_queue(physical_device.queue_family_idx, 0);
