	...
	where DS: pipeline::DynamicState,
{
	pub fn set_dynamic_state(
		...
		L,
		InRenderPass<Inline, Sp, PSp>,
		PipelineStater<SLs, SL, PCs, PC, DSs, DSs>
	> {
...
impl<L, State, SLs, SL, PCs, PC, DSs, DS> CommandBuffer<
	L,
	State,
	PipelineStater<SLs, SL, PCs, PC, DSs, DS>
> {
	pub fn bind_descriptor_set(self) {}
}

impl<Sp, PSp, D, P, PS> CommandBuffer<
	Primary,
...
impl<SLs, SL, PCs, PC1, DSs, DS1> PipelineStater<SLs, SL,PCs, PC1, DSs, DS1> {
	fn into<PC2, DS2>(self) -> PipelineStater<SLs, SL, PCs, PC1, DSs, DS2> {
		PipelineStater {
